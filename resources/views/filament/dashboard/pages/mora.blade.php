...
<div class="flex flex-col">
    ...
    <form method="GET" action="{{ route('moras.exportar.pdf') }}" class="p-6 flex flex-col gap-6">
        ...
        <!-- Tabla optimizada para mejor visualización -->
        <div class="overflow-x-auto">
            <table class="w-full min-w-[1400px] mt-6 bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow text-sm leading-tight">
                <thead class="bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white text-center">
                    <tr>
                        <th class="px-3 py-3 font-semibold min-w-[140px]">Nombre del Grupo</th>
                        <th class="px-2 py-3 font-semibold min-w-[80px]">N° Integrantes</th>
                        <th class="px-2 py-3 font-semibold min-w-[70px]">N° Cuota</th>
                        <th class="px-3 py-3 font-semibold min-w-[100px]">Monto de Cuota</th>
                        <th class="px-3 py-3 font-semibold min-w-[110px]">Fecha Vencimiento</th>
                        <th class="px-3 py-3 font-semibold min-w-[130px]">Saldo de cuota pendiente</th>
                        <th class="px-2 py-3 font-semibold min-w-[80px]">Días de Atraso</th>
                        <th class="px-3 py-3 font-semibold min-w-[120px]">Monto Mora pendiente</th>
                        <th class="px-3 py-3 font-semibold min-w-[120px]">Monto Mora Pagada</th>
                        <th class="px-3 py-3 font-semibold min-w-[120px]">Monto total a pagar</th>
                        <th class="px-2 py-3 font-semibold min-w-[80px]">Estado</th>
                        <th class="px-3 py-3 font-semibold min-w-[130px]">Acciones</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($cuotas_mora as $cuota)
                        <tr class="transition duration-150 text-left border-b border-blue-100 dark:border-gray-700 bg-white dark:bg-gray-800 hover:bg-blue-50 dark:hover:bg-gray-800">
                            <td class="px-3 py-3 text-gray-800 dark:text-white font-medium">{{ $cuota->prestamo->grupo->nombre_grupo ?? 'N/A' }}</td>
                            <td class="px-2 py-3 text-gray-800 dark:text-white text-center">{{ $cuota->prestamo->grupo->clientes()->count() ?? 0 }}</td>
                            <td class="px-2 py-3 text-gray-800 dark:text-white text-center">{{ $cuota->numero_cuota }}</td>
                            <td class="px-3 py-3 text-blue-700 dark:text-blue-200 font-semibold">S/ {{ number_format($cuota->monto_cuota_grupal, 2) }}</td>
                            <td class="px-3 py-3 text-gray-800 dark:text-white text-center">{{ \Carbon\Carbon::parse($cuota->fecha_vencimiento)->format('d/m/Y') }}</td>
                            <td class="px-3 py-3 text-gray-800 dark:text-white">
                                ...
                                @else
                                    <span class="font-semibold">S/ {{ number_format($cuota->saldo_pendiente, 2) }}</span>
                                @endif
                            </td>
                         <td class="px-2 py-3 text-red-600 dark:text-red-300 font-bold text-center">
                                @if($cuota->mora)
                            ...
                            <td class="px-3 py-3 text-pink-700 dark:text-pink-200 font-semibold">
                                @if($cuota->mora)
                                    @php
                            ...
                            <td class="px-3 py-3 text-purple-700 dark:text-purple-200 font-semibold">
                                @if($cuota->mora)
                                    @php
                            ...
                            <td class="px-3 py-3 text-green-700 dark:text-green-200 font-bold">
                                @php
                            ...
                            <td class="px-2 py-3 text-gray-800 dark:text-white text-center">
                                @if($cuota->mora)
                                    <span class="px-2 py-1 rounded text-xs font-semibold bg-red-200 text-red-800 dark:bg-red-700 dark:text-white">
                                        {{ ucfirst(str_replace('_', ' ', $cuota->mora->estado_mora)) }}
                                    </span>
                                @else
                                    <span class="px-2 py-1 rounded text-xs font-semibold bg-gray-200 text-gray-800 dark:bg-gray-600 dark:text-white">
                                        Sin mora
                                    </span>
                                @endif
                            </td>
                            <!-- Columna de Acciones optimizada -->
                    <td class="px-3 py-2 text-gray-800 dark:text-white">
                        @if($cuota->mora && in_array($cuota->mora->estado_mora, ['pendiente', 'parcial', 'parcialmente_pagada']))
                            @if(auth()->user()->hasRole('Jefe de creditos'))
                                <!-- Mensaje para Jefe de Crédito -->
                                <div class="inline-flex items-center px-3 py-2 text-xs font-semibold rounded-lg shadow
                                            bg-yellow-100 text-yellow-800
                                            dark:bg-yellow-600 dark:text-yellow-100
                                            border border-yellow-300 dark:border-yellow-500">
                                    <svg class="w-3 h-3 mr-1 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        @elseif($cuota->mora && $cuota->mora->estado_mora === 'pagada')
                            <span class="inline-flex items-center px-2 py-1 rounded bg-green-100 text-green-800 font-semibold text-xs">
                                <svg class="w-3 h-3 mr-1 text-green-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    ...
                    @empty
                        <tr>
                            <td colspan="12" class="px-4 py-6 text-center text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 rounded-b-lg">No hay cuotas en mora.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
