...
class GrupoDetallePagos extends Page implements HasTable
{
                                    ...
                                    \Filament\Forms\Components\Grid::make(2)
                                        ->schema([
                                            ...
                                            \Filament\Forms\Components\TextInput::make('monto_pagado')
                                                ...
                                                ->live(onBlur: true)
                                                ->afterStateUpdated(function ($state, callable $set, callable $get, $record) {
                                                    if (! $record || ! $record->cuotaGrupal || in_array($get('tipo_pago'), ['pago_completo', 'pago_parcial'])) {
                                                        return;
                                                    }
     ...
     *
     * Para préstamos parcialmente retanqueados, estado_cuota_grupal puede no
     * reflejar aún la cobertura aplicada, por lo que se replica la misma lógica
     * usada en PagoResource: candidatas = cuotas con estado_pago != 'pagado',
     * y la vigente es la primera con saldoPendiente() > 0 (ver Scenario 5.6).
     */
    protected function getCuotaGrupalIdVigente()
    {
        if ($this->prestamo->es_parcialmente_retanqueado) {
            $cuotas = $this->prestamo->cuotasGrupales()
                ->where('estado_pago', '!=', 'pagado')
                ->orderBy('numero_cuota')
                ->get();

            $cuota = $cuotas->first(fn ($c) => $c->saldoPendiente() > 0);
